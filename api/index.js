...
router.get("/get-channel-videos", (req, res) => {
    ...
    connect.query(
        "select * from video where channel_id = '" + channel_id + "' order by release_date DESC",
...
router.post("/write-viewed-video_id", (req, res) => {
    const viewed_video_ids = req.query.viewed_video_ids;
    const email = req.query.email;
    connect.query("update `user` set viewed_video_ids = '" + viewed_video_ids + "' where email = '" + email + "'", function (error, results, fields) {
        if (error) {
